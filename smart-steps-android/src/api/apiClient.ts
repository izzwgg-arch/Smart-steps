...
class ApiClient {
  ...
  constructor() {
    ...
    this.client = axios.create({
      ...
      // Required for NextAuth cookie-based sessions
  ...
  // NOTE: No submit endpoint — server status flow is DRAFT → APPROVED directly.
  async approveTimesheet(id: string) {
    const endpoint = getEndpoint('timesheets', 'approve', id);
    return this.post(endpoint);
  }

  async unapproveTimesheet(id: string) {
    const endpoint = getEndpoint('timesheets', 'unapprove', id);
    return this.post(endpoint);
  }
