...
export function TimesheetsList({ isArchive = false }: { isArchive?: boolean }) {
  ...
  const fetchTimesheets = async () => {
    try {
      let url = `/api/timesheets?page=${page}&limit=${rowsPerPage}&search=${debouncedSearchTerm}&isBCBA=false`
      if (isArchive) {
        url += `&archived=true`
      }
      if (selectedUserId) {
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {timesheets.map((timesheet) => (
              <tr key={timesheet.id} className="hover:bg-gray-50">
                ...
                <td className="px-3 py-2 text-xs text-gray-500 overflow-hidden text-ellipsis max-w-[150px]">
                  {timesheet.bcba.name}
