...
async function testArchiveQuery() {
  try {
    ...
    const invoicedCount = await prisma.timesheet.count({
      where: {
        deletedAt: null,
        invoicedAt: { not: null },
      },
    ...
    const regularInvoicedCount = await prisma.timesheet.count({
      where: {
        deletedAt: null,
        invoicedAt: { not: null },
        isBCBA: false,
    ...
    const bcbaInvoicedCount = await prisma.timesheet.count({
      where: {
        deletedAt: null,
        invoicedAt: { not: null },
        isBCBA: true,
    ...
    // Test 4: Get a sample
    const sample = await prisma.timesheet.findFirst({
      where: {
        deletedAt: null,
        invoicedAt: { not: null },
      },
