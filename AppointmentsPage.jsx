...
/* ─── AppointmentCard ───────────────────────────────────────────────────────── */
// Three visual tiers based on pixel height:
//  TINY   < 26px  → single flex row: client name · time     (no resize handle — too small)
//  SMALL  < 48px  → two rows: client + time / service + badge
//  NORMAL ≥ 48px  → full stacked: client → service → time → status
//
...
const AppointmentCard = memo(function AppointmentCard({ appt, onClick, onResizeSave, onMoveDragStart, isDragging }) {
  ...
  /* ── TINY: single horizontal row — no resize handle (insufficient height) ── */
  if (cardH < 26) {
    return (
      <div {...wrapProps}>
        <div className="flex h-full items-center gap-1 px-1.5 overflow-hidden">
          {cfg && (
            <span
              className={`shrink-0 rounded px-1 py-px text-[8px] font-bold uppercase leading-none ${cfg.cls}`}
          ...
          <span
            className="min-w-0 flex-1 truncate text-[10px] font-bold leading-none"
          ...
          <span
            ...
            style={{ color: timeColor, whiteSpace: "nowrap", opacity: 0.85 }}
          >
            {timeStr}
          </span>
        </div>
  ...
  /* ── SMALL: two compact rows ──────────────────────────────────────────────── */
  if (cardH < 48) {
    return (
      <div {...wrapProps}>
        <div className="px-2 py-0.5 space-y-px">
          {/* Row 1: client name (primary) + start time */}
          <div className="flex items-center justify-between gap-1 overflow-hidden">
            ...
            <span
              className="shrink-0 text-[9px] font-bold leading-tight"
              style={{ color: timeColor, whiteSpace: "nowrap", opacity: 0.85 }}
            >
              {timeStr}
            </span>
          </div>
          {/* Row 2: service name + badge */}
          <div className="flex items-center gap-1.5 overflow-hidden">
            {client && (
              <span
                className="min-w-0 flex-1 truncate text-[10px] font-bold leading-tight"
                style={{ color: svcColor, opacity: 0.8 }}
              >
                {service}
              </span>
            )}
  ...
  /* ── NORMAL: full stacked layout ──────────────────────────────────────────── */
  // Order: client name → service → time range → phone → status
  return (
    <div {...wrapProps}>
      <div className="px-2 py-1 space-y-px">
        {/* 1. Client name */}
        <p className="truncate text-[12px] font-bold leading-tight" style={{ color: nameColor }}>
          {client || service}
        </p>
        {/* 2. Service name */}
        {client && (
          <p className="truncate text-[10px] font-bold leading-tight" style={{ color: svcColor, opacity: 0.85 }}>
            {service}
          </p>
        )}
        {/* 3. Time range — updates live during resize drag */}
        <p className="truncate text-[10px] font-bold leading-tight" style={{ color: timeColor, opacity: 0.75 }}>
          {timeRange}
        </p>
        {/* 4. Client phone — only when card is tall enough to show it comfortably */}
        {clientPhone && cardH >= 68 && (
          <p className="truncate text-[10px] leading-tight" style={{ color: timeColor, opacity: 0.65 }}>
            📞 {clientPhone}
          </p>
        )}
        {/* 5. Status badge */}
        {cfg && (
          <span
            className={`mt-0.5 inline-block max-w-full truncate rounded px-1 py-px text-[9px] font-bold uppercase tracking-wide leading-tight ${cfg.cls}`}
          >
            {cfg.label}
          </span>
        )}
...
function AppointmentModal({ modal, onClose, onSaved, onDeleted, clients, services, providers, toast }) {
  ...
  async function handleSave(e) {
    ...
    try {
      ...
      const selClient   = clients.find((c) => c.id === form.clientId)   ?? null;
    ...
    <div className="fixed inset-0 z-50 flex items-start justify-center overflow-y-auto p-4 pt-10">
      ...
      <div className="relative z-10 w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="flex border-b border-slate-100 px-6">
          {[{ key:"service",label:"Service" },{ key:"event",label:"Event" }].map(({ key, label }) => (
