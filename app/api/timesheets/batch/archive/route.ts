...
/**
 * POST /api/timesheets/batch/archive
 * Batch archive/unarchive timesheets
 */
export async function POST(request: NextRequest) {
  try {
    ...
    // Update timesheets (only non-deleted, regular timesheets - not BCBA)
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully ${archived ? 'archived' : 'unarchived'} ${count} timesheet(s)`,
      count: count,
    })
  } catch (error: any) {
